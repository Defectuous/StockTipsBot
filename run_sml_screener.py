...
def _compute_buy_amount(screener_id: str) -> float:
    """Return the per-trade cash amount based on today's deployable capital."""
...
def _maybe_reset_day(
    ...
    """Reconcile wallet at the start of each new ET trading day."""
...
def scan_and_trade(
    ...
    for stock in passing:
        # Re-check cap inside loop (we may have bought one already this cycle)
        ...
        if HARD_STOP_PCT > 0:
            ...
            if hs_order:
                ...
                logger.info("  STOP  %s  hard=-%.1f%% ($%.4f)  id=%s",
                            sym, stop_pct, hard_stop_price, hs_order.id)
            else:
                ...
                logger.warning("  Hard stop failed for %s — falling back to trailing stop at %.1f%%", sym, stop_pct)
                ts_order = trader.submit_trailing_stop(sym, fill_qty, stop_pct)
                if ts_order:
                    update_trailing_stop_order(pos_id, str(ts_order.id))
                    logger.info("  STOP  %s  trail=%.1f%%  id=%s", sym, stop_pct, ts_order.id)
                else:
...
def main():
    ...
    # Seed wallet on first run
    init_wallet(SCREENER_ID, STARTING_BALANCE)

    # last_day[0] is a mutable holder so _maybe_reset_day can update it
    last_day = [None]
